/** Order creation + status transitions (ported from backend/src/services/orders.ts). */
...
export type CreateOrderResult = {
  ...
  /** Plaintext temp password for the credentials email. Never logged in prod. */
...
async function resolveOrderUser(
  ...
  // Strong random temp password (16+ chars of entropy).
...
export async function updateOrderStatus(orderId: string, status: DeliveryStatus, note?: string) {
  return prisma.$transaction(async (tx) => {
    ...
    return tx.order.update({
      where: { id: orderId },
      data: { status, statusHistory: { create: { status, note: note ?? null } } },
    });
  });
}
