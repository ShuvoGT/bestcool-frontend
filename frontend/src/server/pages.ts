/**
 * CMS block enrichment + current flash sale (ported from
 * backend/src/services/pages.ts). Resolves product/category ids in blocks to
 * full objects so the storefront renders without extra round-trips.
 */
    ...
    blocks.map(async (block) => {
      ...
      switch (block.type) {
        case "FEATURED_PRODUCTS": {
          const ids = ((block.content as Record<string, unknown>)?.productIds as string[]) ?? [];
        ...
        case "FEATURED_CATEGORIES": {
          const ids = ((block.content as Record<string, unknown>)?.categoryIds as string[]) ?? [];
