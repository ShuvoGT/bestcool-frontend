/**
 * Pathao Courier Merchant API (ported verbatim from backend/src/couriers/pathao.ts).
 * OAuth password grant → access token (cached), then create order / query.
 * Addresses require numeric city/zone ids (form collects a zone id; defaults Dhaka).
 */
import type {
  CourierConfig,
  CourierProvider,
