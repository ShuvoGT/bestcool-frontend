/**
 * Courier orchestration (ported from backend/src/services/couriers.ts): create a
 * consignment for an order, refresh its status, and apply verified webhook updates.
 *
 * Rules:
 *  - On successful consignment, the order's courier fields are saved and the
 *    delivery status moves to SHIPPED (fires the email/SMS with the tracking id).
 *  - Status refresh / webhook pull the courier's latest status; "delivered"
 *    advances the order to DELIVERED, "cancelled/returned" cancels it (restoring
 *    stock via updateOrderStatus). Never downgrades a terminal order state.
 */
...
import { getCourier } from "./providers";
import { loadCourierConfig } from "./config";
import type { CreateParcelInput } from "./CourierProvider";

// Re-export so existing `@/server/couriers` imports keep working.
export { loadCourierConfig } from "./config";
export { listCouriers, getCourier } from "./providers";
...
export async function sendToCourier(orderId: string, input: SendToCourierInput) {
  ...
  // Save courier fields first — the SHIPPED email/SMS reads them.
 ...
 * DELIVERED when the courier confirms delivery. Never downgrades a terminal state.
 */
async function applyCourierStatus(
  ...
  // Only notify when the order ACTUALLY transitioned — updateOrderStatus returns
  // the (unchanged) order on a no-op and throws on an illegal move, so a racing/
  // duplicate webhook can't trigger a false "delivered/cancelled" SMS.
