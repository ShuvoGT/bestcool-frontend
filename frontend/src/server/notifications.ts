/**
 * Customer notifications (ported from backend/src/services/notifications.ts):
 * transactional emails + SMS for order placed, account credentials, status
 * changes, payment confirmation. Everything here is fire-and-forget — a
 * mail/SMS failure never breaks order placement or status updates.
 */
    ...
    if (tempPassword) {
      await sendMail(
        ...
        shell(
          ...
          <a href="${SITE_URL}/account/login" style="display:inline-block;background:#2563eb;color:#fff;padding:10px 22px;border-radius:8px;text-decoration:none;font-size:14px;font-weight:bold">Log in to your account</a>`,
        ),
      );
    }
    ...
    await sendMail(
      ...
      shell(
        ...
        `<p style="color:#52525b;font-size:14px;line-height:1.6">Hi ${order.shippingName}, ${copy.line}</p>${trackingHtml}
         ...
         <a href="${SITE_URL}/account/orders/${order.orderNumber}" style="display:inline-block;background:#2563eb;color:#fff;padding:10px 22px;border-radius:8px;text-decoration:none;font-size:14px;font-weight:bold">View order</a>`,
      ),
    );
