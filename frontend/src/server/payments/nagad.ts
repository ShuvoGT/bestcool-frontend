/**
 * Nagad Payment Gateway (ported verbatim from backend/src/payments/nagad.ts).
 * initialize → complete (redirect) → customer pays → verify() GETs
 * verify/payment/{ref} server-side and trusts only "Success". Sensitive
 * payloads are RSA-encrypted/signed and response signatures verified.
 */
import type {
  InitiateContext,
  InitiateResult,
