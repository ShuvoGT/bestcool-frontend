/**
 * SSLCommerz hosted checkout (ported verbatim from backend/src/payments/sslcommerz.ts).
 * initiate → customer pays → gateway POSTs success/fail/cancel/IPN with a
 * val_id → verify() calls the Validation API server-side (never trust redirect).
 */
import type {
  InitiateContext,
  InitiateResult,
